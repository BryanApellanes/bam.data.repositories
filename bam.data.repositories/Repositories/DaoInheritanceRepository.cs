...
namespace Bam.Net.Data.Repositories
{
    ...
    public class DaoInheritanceRepository : DaoRepository
    {
        ...
        public override object Create(object toCreate)
        {
            ...
            List<ISqlStringBuilder> sqls = SqlWriter.GetInsertStatements(toCreate, Database); // due to supporting some database types that don't allow for (or make it easy to do) multiline scripts or multi table inserts, each insert is returned as a separate sqlstring builder
            ...
            sqls.Rest(1, sql =>
            {
                ...
                Database.ExecuteSql(sql.ToString(), dbParams.ToArray());
            });
            // - end TODO            
        ...
        protected void SaveChildCollections(object parent)
        {
            ...
            {
                List<ISqlStringBuilder> sqls = new List<ISqlStringBuilder>();
        ...
        protected void Execute(ISqlStringBuilder sql)
        {
