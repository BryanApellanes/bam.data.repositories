...
namespace Bam.Data.Repositories
{
    ...
    public abstract class CompositeKeyAuditRepoData : AuditRepoData, IHasKeyHash, IHasKey
    {
        ...
        public bool ExistsIn<T>(IRepository repository, out T? existing) where T: class, new()
        {
            existing = LoadByCompositeKey<T>(repository);
            return existing != null;
        }
        
        public virtual T? LoadByCompositeKey<T>(IRepository repository) where T : class, new()
        {
            return repository.Query<T>(new { CompositeKey }).FirstOrDefault();
        }

        public virtual T? LoadByCompositeKeyId<T>(IRepository repository) where T : class, new()
        {
            return repository.Query<T>(new { CompositeKeyId }).FirstOrDefault();
        }
        
        public virtual string GetCompositeKeyString(HashAlgorithms algorithm = HashAlgorithms.SHA256)
        {
